...
use App\Http\Requests\Template\Section\CalltoactionSectionRequest;
...
class CalltoactionSectionController extends Controller
{
     ...
     */
    public function index(CalltoactionSectionRequest $request)
    {
        $limit = $request->query('limit', 10);
        $section = CalltoactionSection::paginate($limit);
     ...
     */
    public function store(CalltoactionSectionRequest $request)
    {
    ...
    {
        $section = CalltoactionSection::find($id);

        if (!$section) {
            return $this->sendError("Not Found", "Calltoaction Section Not found", Response::HTTP_NOT_FOUND);
        }
     ...
     */
    public function update(CalltoactionSectionRequest $request, string $id)
    {
        ...
        $section = CalltoactionSection::where('id', $id)->first();
        $data = new SectionCollection(collect($section));
